...
async function handleApi(req, res, pathname, query) {
  ...
  if (req.method === 'GET' && pathname === '/api/health') {
    ...
    const onlineCount = uniqueAccounts.size;
     ...
     return sendJson(res, 200, {
      ...
      webSocket: true,
      wsTransportPath: '/',
      wsChunkReassembly: true,
      wsShortTokenAuth: true,
      wsPureJoin: true,
      longPoll: false,
      wsCount,
      lpCount,
  ...
  if (req.method === 'GET' && pathname === '/api/auth/me') {
    ...
    // v4.4.16：刷新進程內 WS token 快取（雙保險；主通道已改為 URL query 無狀態認證，不依賴此快取）
    try {
      const _bearer = req.headers['authorization'] || '';
      if (_bearer.startsWith('Bearer ') && global._wsSetAccountToken) global._wsSetAccountToken(accName, _bearer.slice(7));
    } catch (e) {}
...
(function bootstrap() {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log('========================================');
    console.log('  King v4.4.19 · 正式營運伺服器');
    console.log('========================================');
