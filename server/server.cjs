...
function serveStatic(req, res, pathname) {
  ...
  try {
    ...
    const dlFiles = {
      '/game-code.zip': 'monarch-blade-v2.5.1-code.zip',
      '/game-code-2.5.1.zip': 'monarch-blade-v2.5.1-code.zip',
      '/assets-part1.zip': 'monarch-blade-v2.5.1-assets-part1.zip',
      '/assets-part2.zip': 'monarch-blade-v2.5.1-assets-part2.zip',
    };
    if (dlFiles[pathname]) {
    ...
    if (pathname === '/PARTS-MANIFEST.txt') {
      headers['Content-Disposition'] = 'attachment; filename="PARTS-MANIFEST-v2.5.1.txt"';
    }
...
async function handleApi(req, res, pathname, query) {
  ...
  if (req.method === 'GET' && pathname === '/api/health') {
    ...
    return sendJson(res, 200, {
      ...
      version: '2.5.1',
      build: '2.5.1-2608281801',
      buildId: '2.5.1-2608281801',
      time: Date.now(),
  ...
  if (req.method === 'GET' && pathname === '/api/selftest') {
    ...
    try {
      ...
      checks.push({
        name: 'server',
        pass: true,
        detail: { version: '2.5.1', uptimeMs: Math.floor(process.uptime() * 1000), platform: process.platform, nodeVersion: process.version, pid: process.pid },
        ms: Date.now() - t0,
      });
    } catch(e) {
  ...
  // v2.5.1：前端創角後 / 選角時必須靠這支載入 saveData 進遊戲
...
<title>伺服器自體檢查 · 君主之刃 v2.5.1</title>
...
<div class=\"wrap\">
  <div class=\"header\">
    ...
    <div class=\"subtitle\">v2.5.1 · 即時驗證伺服器與遊戲功能</div>
  </div>
  ...
  <div class=\"footer\">君主之刃 v2.5.1 · 伺服器自體檢查</div>
</div>
<script>
...
(function bootstrap() {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log('========================================');
    console.log('  君主之刃 v2.5.1 · 正式營運伺服器');
    console.log('========================================');
