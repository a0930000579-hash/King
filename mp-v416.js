(function() {
  ...
  function _handleClientChunk(msg) {
    try {
      ...
      buf.received++;
      _wsDiag('[v4.1.7] 收到chunk cid=' + cid + ' idx=' + idx + '/' + total + ' dataLen=' + data.length);
      if (buf.received >= total) {
        ...
        _wsDiag('[v4.1.7] chunk組裝完成 cid=' + cid + ' fullLen=' + full.length);
        try {
          return JSON.parse(full);
        } catch(e) {
          console.error('[GAME-WS] chunk組裝後JSON失敗:', e.message, '前50字=', full.substring(0,50));
          _wsDiag('[v4.1.7] chunk組裝後JSON失敗: ' + e.message);
          return null;
        }
      }
      return null; // 還沒收齊
