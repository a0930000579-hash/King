  ...
  if (backend === 'postgres') {
    ...
    return {
      player: {
        ...
        level: sd.level || 1,
        exp: sd.exp || 0,
        hp: 100, mp: 80, hpMax: 100, mpMax: 80, maxHp: 100, maxMp: 80, expMax: 100,
        atk: 10, def: 5, spd: 1, crit: 5, hit: 95, dodge: 5,
        state: 'idle', facing: 'right', skillCooldowns: [0,0,0,0,0,0,0,0], buffs: {},
        x: 0, y: 0, targetX: 0, targetY: 0, direction: 'down',
      },
      resources: { gold: 300000, gem: 3000000 },
      inventory: [],
      ownedHeroes: [],
      ownedPets: [],
      ownedTransforms: [],
...
async function createCharacter(account, serverId, charIdx, name, classId, saveData) {
  ...
  const fullSave = {
    player: {
      ...
      level: 1,
      exp: 0,
      expMax: 100, // v4.4.15：補客戶端欄位，避免重登 exp/expMax=NaN%
      hp: cls.hp, mp: cls.mp,
      hpMax: cls.hp, mpMax: cls.mp, maxHp: cls.hp, maxMp: cls.mp, // 新舊命名同時寫入
      atk: cls.atk, def: cls.def, spd: cls.spd,
      crit: cls.crit, hit: cls.hit, dodge: cls.dodge,
      state: 'idle', facing: 'right',
      skillCooldowns: [0,0,0,0,0,0,0,0], buffs: {},
      x: 0, y: 0, targetX: 0, targetY: 0, direction: 'down',
    },
    // v4.4.15：初始資源與客戶端一致，新角重登不再歸 0
    resources: { gold: 300000, gem: 3000000 },
    inventory: [],
    ownedHeroes: [],
    ownedPets: [],
